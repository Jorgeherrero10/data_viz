...
# --- Helper Functions ---
def reset_filters_d2():
    # Define a function to reset filters if needed, currently does nothing specific
    # but could be used to set session state defaults for d2
    pass

def reset_filters_d3():
    # Define a function to reset filters if needed for d3
    pass
...
def display_dashboard2_sidebar():
    with st.sidebar:
        ...
        # Use unique keys for widgets to avoid conflicts if names are the same across pages
        ...
        incentive_level = st.select_slider(
            ...
            options=incentive_levels_d2,
            value="Full Incentives",
            key="d2_incentive_level"
        )
    ...
    with row2_col2:
        ...
        if selected_tab == "Implementation Strategy":
            ...
            if not ops_df_filtered.empty:
                fig_ops.add_shape(
                    type="line", x0=implementation_timeline, y0=0, x1=implementation_timeline, y1=10,
                    line=dict(color="blue", width=2, dash="dot")
                )
                fig_ops.add_annotation(
                    x=implementation_timeline, y=9.5, text=f"Target: {implementation_timeline} mo",
                    showarrow=True, arrowhead=1, ax=20, ay=-20, font=dict(size=10)
                )
                fig_ops.update_traces(
                    textposition='top center', marker=dict(opacity=0.8, line=dict(width=1, color='black')),
                    textfont=dict(size=8)
                )
            fig_ops.update_layout(
                title="Implementation Strategy by District", xaxis_title=None, yaxis_title="Priority Score",
                legend_title="Incentive Level", height=250, margin=dict(l=30, r=30, t=40, b=30),
            ...
            affordability_data = pd.DataFrame({
                "Tenant Category": tenant_categories_d2, "Before Control (%)": before_burden_d2,
                "After Control (%)": after_burden_d2, "Improvement (%)": improvement_d2
            })
            ...
            if not affordability_data_filtered.empty:
                fig_afford.add_trace(go.Bar(
                    x=affordability_data_filtered["Tenant Category"], y=affordability_data_filtered["Before Control (%)"],
                    name="Before", marker_color="#d62728", text=affordability_data_filtered["Before Control (%)"].apply(lambda x: f"{x:.1f}%"),
                    textposition="auto", textfont=dict(size=9)
                ))
                fig_afford.add_trace(go.Bar(
                    x=affordability_data_filtered["Tenant Category"], y=affordability_data_filtered["After Control (%)"],
                    name="After", marker_color="#2ca02c", text=affordability_data_filtered["After Control (%)"].apply(lambda x: f"{x:.1f}%"),
                    textposition="auto", textfont=dict(size=9)
                ))
                for i, cat in enumerate(affordability_data_filtered["Tenant Category"]):
                    ...
                    fig_afford.add_annotation(
                        x=cat, y=after_burden_d2[idx] - 3, text=f"↓{improvement_d2[idx]:.1f}%",
                        showarrow=False, font=dict(size=9, color="black")
                    )
            fig_afford.update_layout(
                title="Impact on Housing Affordability", xaxis_title=None, yaxis_title="% Income on Rent",
                barmode="group", yaxis=dict(range=[0, max(before_burden_d2) * 1.1]), height=250,
                margin=dict(l=30, r=30, t=40, b=30), font=dict(size=10),
            ...
            if not categories_to_show: # Default if none selected from filter
                categories_to_show = [("Youth Before", "#ff9999"), ("Youth After", "#99ff99"), ("Low-Income Before", "#ffcc99"), ("Low-Income After", "#99ccff")]

            fig_district = go.Figure()
            if not district_impact.empty:
                # Add trace for each category
                for cat, color in categories_to_show:
                    if cat in district_impact.columns: # Check if column exists
                         fig_district.add_trace(go.Bar(
                            x=district_impact["District"], y=district_impact[cat], name=cat, marker_color=color,
                            text=district_impact[cat].apply(lambda x: f"{x:.1f}%"), textposition="auto", textfont=dict(size=9)
                         ))
            fig_district.update_layout(
                title="District-Specific Affordability Impact", xaxis_title=None, yaxis_title="% Income on Rent",
                barmode="group", height=250, margin=dict(l=30, r=30, t=40, b=30), font=dict(size=10),
                legend=dict(orientation="h", y=1.02, x=1, font=dict(size=9))
            )
...
def display_dashboard3_sidebar():
    with st.sidebar:
        ...
        incentive_budget = st.slider(
            "Incentive Budget (% of total)", min_value=20.0, max_value=40.0, value=27.8, step=0.1,
            help="Percentage of total housing budget allocated to incentives", key="d3_incentive_budget"
        )
        tax_savings = st.slider(
            "Tax Savings per Landlord (€)", min_value=5000, max_value=9000, value=7313, step=100,
            help="Average tax savings per participating landlord", key="d3_tax_savings"
        )
        growth_rate_without = st.slider(
            "Base Growth Rate (%)", min_value=1.0, max_value=3.0, value=2.0, step=0.1,
            help="Annual growth rate without program", key="d3_growth_without"
        ) / 100
        growth_rate_with = st.slider(
            "Enhanced Growth Rate (%)", min_value=growth_rate_without*100 + 0.1, max_value=5.0,
            value=3.0, step=0.1, help="Annual growth rate with program", key="d3_growth_with"
        ) / 100
        selected_districts = st.multiselect(
            "Select Districts for Analysis", districts_d3, default=[],
            help="Select specific districts to focus the analysis on", key="d3_districts"
        )
        analysis_view = st.radio(
            "District Analysis View", ["Quadrant Analysis", "Heatmap Analysis"], index=0, key="d3_analysis_view"
        )
...
def display_dashboard3_content(incentive_budget, tax_savings, growth_rate_without, growth_rate_with, selected_districts, analysis_view):
    ...
    # Ensure incentive_budget is not zero for ROI/Payback calculation
    ...
    if years_to_payback >= len(updated_cumulative_revenue_diff):
        updated_payback_period = 10.0 + (safe_incentive_budget - updated_cumulative_revenue_diff[-1]) / (updated_revenue_diff[-1] if updated_revenue_diff[-1] != 0 else 1) if len(updated_cumulative_revenue_diff)>0 else 10.0 # Extrapolate if not paid back in 10 years
        updated_payback_period = max(10.0, updated_payback_period) # Cap at a reasonable upper bound if extrapolation is huge
    else:
    ...
    updated_payback_period = min(updated_payback_period, 50) # Cap payback period at 50 years for sanity
    ...
    # --- KPIs Display ---
    kpi_placeholder = st.empty() # Use a placeholder to ensure KPIs are drawn above charts
    ...
    with row1_col1:
        ...
        fig_sankey.add_annotation(
            x=0.5, y=-0.15, xref='paper', yref='paper', text=f'<b>{incentive_budget:.1f}%</b> of the total budget is allocated to tax incentives', showarrow=False, font=dict(size=14, color='black'), align='center', bgcolor='rgba(220, 239, 110, 0.8)', bordercolor='#3498db', borderwidth=1, borderpad=8, opacity=0.9
        )
    ...
    with row1_col2:
        ...
        fig_waterfall.update_layout(
            title=dict(text='Landlord Financial Impact Analysis (Per Property)', x=0.5, font=dict(size=16), xanchor='center', yanchor='top'),
    ...
    with row2_col1:
        ...
        if chart_type == "Long-term Projection":
            ...
            fig_combined.update_layout(
                title=dict(text='Long-term Property Tax Revenue Projection (2025–2034)', x=0.5, y=0.98, font=dict(size=16), xanchor='center', yanchor='top'),
            ...
            fig_combined.add_annotation(
                x=2030, y=max(updated_revenue_diff or [0]) * 1.3, text=f"<b>Cumulative 10-Year Gain:</b><br>+€{updated_cumulative_diff:.1f}M", showarrow=False, font=dict(size=11, color='orange'), bgcolor='white', bordercolor='orange', borderwidth=1, borderpad=5
            )
            ...
            fig_combined.update_layout(
                title=dict(text='Correlation: Tax Incentives & Landlord Participation', x=0.5, font=dict(size=16), xanchor='center', yanchor='top'),
                xaxis_title='Tax Incentive Amount (€)', yaxis_title='Landlord Participation Rate (%)', height=310, margin=dict(l=40, r=20, t=40, b=40), plot_bgcolor='white',
            ...
            if analysis_view == "Quadrant Analysis":
                ...
                fig_combined = px.scatter(
                    ...
                    labels={'Incentive Cost (€/unit)': 'Incentive Cost (€/unit)', 'Affordability Improvement (%)': 'Affordability Improvement (%)', 'Implementation Complexity': 'Complexity (1–5)'}
                ...
                fig_combined.update_layout(
                    ...
                    xaxis=dict(title='Incentive Cost (€/unit)', gridcolor='lightgrey', zeroline=False, range=[0, x_max]),
                ...
                if not filtered_district_analysis.empty:
                    ...
                    fig_combined = go.Figure() # Empty figure if no data

                fig_combined.update_layout(
                    title=dict(text='District-Level Affordability vs. Costs', x=0.5, font=dict(size=16), xanchor='center', yanchor='top'),
...
# Apply CSS based on the current page
if st.session_state.page == 'dashboard2':
...
nav_cols = st.columns([1, 1, 5, 1, 1]) # Create columns for alignment

with nav_cols[1]:
    if st.session_state.page == 'dashboard3':
